...
open class VariableBlurUIView: UIVisualEffectView {
    ...
    public init(maxBlurRadius: CGFloat = 20, direction: VariableBlurDirection = .blurredTopClearBottom, startOffset: CGFloat = 0) {
        ...
        let variableBlur = CAFilter.self.perform(NSSelectorFromString(["Type:","With","filter"].reversed().joined()), with: ["Blur","variable"].reversed().joined()).takeUnretainedValue() as? NSObject else {
            return
        }
